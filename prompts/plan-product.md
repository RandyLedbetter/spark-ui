...
## Sprint.yaml Initialization

When creating or updating `docs/sprint.yaml`, use this exact structure:

```yaml
name: [Project Name] - Sprint 1
phase: planning  # planning | implementing | testing | done
current_spec: null
specs: {}        # All specs go here - no other top-level task sections
notes: |
  Planning phase started [date]
```

**Important:** The only valid top-level keys are: `name`, `phase`, `current_spec`, `specs`, `notes`.
Never create other top-level sections (like `foundation:` outside of `specs:`).
