...
When writing acceptance_criteria and other list items, avoid YAML special characters at the start of values:

| Problematic | Safe Alternative |
|-------------|------------------|
| `- 'open' event fires` | `- open event fires` |
| `- [data-close] triggers` | `- data-close attribute triggers` |
| `- size: small renders` | `- size small renders` |

**Rule:** Keep list item values as plain text. Avoid quotes, brackets, and colons at the start of values.
...
## Sprint.yaml Structure Rules

**IMPORTANT:** All tasks MUST be placed under the `specs:` section.

- Each logical grouping of tasks is a "spec" (e.g., `foundation`, `button-component`)
- Never create top-level sections other than: `name`, `phase`, `current_spec`, `specs`, `notes`
- Setup/scaffolding tasks should be their own spec called `foundation` INSIDE `specs:`

**Correct structure:**
```yaml
specs:
  foundation:          # Setup tasks as a spec
    status: ready
    tasks: [...]
  button-component:    # Feature spec
    status: ready
    tasks: [...]
```
...
```yaml
foundation:            # WRONG: top-level section outside specs
  tasks: [...]
specs:
  button-component:
    tasks: [...]
...
- **Setup tasks belong in a `foundation` spec** (under `specs:`, not as a top-level section)
